...
describe MediumUploader do
  ...
  context 'MODE:disable_processing' do
    ...
    context 'in the case of valid file' do
      ...
      it 'should make the file writable only to the owner and not executable' do
        expect(subject).to have_permissions 0644
      end

      it 'should change the file name to an UUID one and a specific directory structure' do
      ...
      it 'should set the file content_type in model' do
        expect(subject.model.content_type).to eq subject.content_type
      end

      it 'should set the file size in model' do
        expect(subject.model.file_size).to eq subject.size
      end

      it 'should set the file name in model' do
        expect(subject.model.file_name).to eq filename
      end

      it 'should store uploaded files in a specific store_dir' do
    ...
    context 'in the case of files with Farsi basename' do
      ...
      it 'should save the Farsi name as the file_name in model' do
    ...
    context 'in the case of invalid files' do
      ...
      it 'store the file should raise an error' do
        expect{
